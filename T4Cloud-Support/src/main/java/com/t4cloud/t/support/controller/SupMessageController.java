...
public class SupMessageController extends T4Controller<SupMessage, ISupMessageService> {
    ...
    public R<SupMessage> detail(SupMessage supMessage, HttpServletRequest req) {
        QueryWrapper<SupMessage> supMessageQueryWapper = T4Query.initQuery(supMessage, req.getParameterMap());
        SupMessage detail = service.getOne(supMessageQueryWapper);
    ...
    public R<List<SupMessage>> list(SupMessage supMessage, HttpServletRequest req) {
        QueryWrapper<SupMessage> supMessageQueryWapper = T4Query.initQuery(supMessage, req.getParameterMap());
        List<SupMessage> list = service.list(supMessageQueryWapper);
    ...
    public R<IPage<SupMessage>> page(SupMessage supMessage,
        ...
        QueryWrapper<SupMessage> supMessageQueryWapper = T4Query.initQuery(supMessage, req.getParameterMap());
        IPage<SupMessage> pages = service.page(new Page<>(pageNo, pageSize), supMessageQueryWapper);
